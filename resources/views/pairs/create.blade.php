...
@section('title', 'Tambah Pasangan Ganda')
@section('page-title', 'Tambah Pasangan Ganda')
@section('page-subtitle', 'Pasangkan dua pemain untuk kategori ganda')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" 
                           class="form-input @error('name') error @enderror" placeholder="Contoh: Kevin / Marcus" required>
                    @error('name')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="player_1_id" name="player_1_id" class="form-input @error('player_1_id') error @enderror" required>
                            ...
                            @foreach($players as $player)
                                <option value="{{ $player->id }}" {{ old('player_1_id') == $player->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select id="player_2_id" name="player_2_id" class="form-input @error('player_2_id') error @enderror" required>
                            ...
                            @foreach($players as $player)
                                <option value="{{ $player->id }}" {{ old('player_2_id') == $player->id ? 'selected' : '' }}>
                                    {{ $player->name }} ({{ $player->gender }})
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <select id="pair_type" name="pair_type" class="form-input @error('pair_type') error @enderror" required>
                        <option value="">-- Pilih Jenis Ganda --</option>
                        <option value="GD_PUTRA" {{ old('pair_type') === 'GD_PUTRA' ? 'selected' : '' }}>Ganda Putra (GD_PUTRA)</option>
                        <option value="GD_PUTRI" {{ old('pair_type') === 'GD_PUTRI' ? 'selected' : '' }}>Ganda Putri (GD_PUTRI)</option>
                        <option value="GD_CAMPURAN" {{ old('pair_type') === 'GD_CAMPURAN' ? 'selected' : '' }}>Ganda Campuran (GD_CAMPURAN)</option>
                    </select>
                    @error('pair_type')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" 
                              class="form-input @error('description') error @enderror" placeholder="Catatan opsional mengenai pasangan ganda ini...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
